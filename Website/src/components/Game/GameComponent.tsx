...
export default class GameComponent extends React.Component<GameProps, GameState> {
    ...
    constructor(props: any) {
        ...
        this.state.InputLog.push("Around you you see: ")
        ...
        if (this.state.exitingDialogue) {
            ...
            if (this.state.inDialogue && this.state.currentDialogueNode !== undefined) {
                ...
                if (Number.isInteger(num)) {
                    ...
                    this.addMessage("ERR: could not read option choice from input")
                }
            } else {
                ...
                switch (inputParts[0]) {
                    case("poi"):
                        ...
                        var num = Number.parseInt(inputParts[1])
                        if (Number.isInteger(num)) {
                            switch (inputParts[2]) {
                                case("interact"):
                                    this.setState({inDialogue: true})
                                    this.InteractPOI(num)
                                    break
                                case("examine"):
                                    this.addMessage(this.state.POIs[num].Info.Examine);
                                    break
                                default:
                    ...
                    case("traversable"):
                        ...
                        if (Number.isInteger(num)) {
                            switch (inputParts[2]) {
                                case("interact"):
                                    this.setState({inDialogue: true})
                                    this.InteractTraversable(num)
                                    break
                                case("examine"):
                                    this.addMessage(this.state.traversables[num].Info.Examine);
                                    break
                                default:
                                    this.addMessage("ERR: Unknown command. Known commands: Interact, Examine")
                            }
                    ...
                    case("item"):
                        ...
                        if (Number.isInteger(num)) {
                            switch (inputParts[2]) {
                                case("pickup"):
                                    this.pickupItem(this.state.items[num].ItemName)
                                    break
                                case("examine"):
                                    this.addMessage(this.state.items[num].info.Examine);
                                    break
                                default:
                    ...
                    case("inventory"):
                        ...
                        if (Number.isInteger(num)) {
                            switch (inputParts[2]) {
                                case("drop"):
                                    this.dropItem(this.state.inventory[num].ItemName)
                                    break
                                case("examine"):
                                    this.addMessage(this.state.inventory[num].info.Examine);
                                    break
                                default:
    ...
    private isItemPacket(input: ItemPacket | POIPacket | undefined): input is ItemPacket {
        if (input === undefined) {
            return false
        } else {
            return "ItemName" in input
        }
    }
